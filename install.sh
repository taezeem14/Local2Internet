...
case $PKG_MANAGER in
    pkg)
        pkg update -y >/dev/null 2>&1 || error "Failed to update package manager"
        ;;
    apt)
        if [ "$EUID" -ne 0 ]; then
            sudo apt update -y >/dev/null 2>&1 || error "Failed to update package manager"
        else
            apt update -y >/dev/null 2>&1 || error "Failed to update package manager"
        fi
        ;;
    pacman)
        if [ "$EUID" -ne 0 ]; then
...
if [ -n "$DEPS_TO_INSTALL" ]; then
    ...
    case $PKG_MANAGER in
        pkg)
            pkg install -y $DEPS_TO_INSTALL >/dev/null 2>&1 || error "Failed to install dependencies"
            ;;
        apt)
            if [ "$EUID" -ne 0 ]; then
                sudo apt install -y $DEPS_TO_INSTALL >/dev/null 2>&1 || error "Failed to install dependencies"
            else
                apt install -y $DEPS_TO_INSTALL >/dev/null 2>&1 || error "Failed to install dependencies"
            fi
            ;;
        pacman)
            if [ "$EUID" -ne 0 ]; then
...
git clone https://github.com/Taezeem14/Local2Internet.git "$INSTALL_DIR" >/dev/null 2>&1 || error "Failed to clone repository"
