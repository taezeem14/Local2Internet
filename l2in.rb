...
class ToolDownloader
  ...
  def self.download_cloudflared
    ...
    def self.download_loclx
      url = case arch
      when /arm(?!64)/ then "https://loclx-client.s3.amazonaws.com/loclx-linux-arm.zip"
      when /aarch64|arm64/ then "https://loclx-client.s3.amazonaws.com/loclx-linux-arm64.zip"
      when /x86_64/ then "https://loclx-client.s3.amazonaws.com/loclx-linux-amd64.zip"
      else "https://loclx-client.s3.amazonaws.com/loclx-linux-386.zip"
      end

      tmp = "#{TOOLS_DIR}/loclx.zip"
      
      return false unless exec_silent("wget -q #{url} -O #{tmp}")
      return false unless exec_silent("unzip -q -o #{tmp} -d #{TOOLS_DIR}")
      
      # Loclx binary might be wrapped in a folder, find it
      loclx_bin = `find #{TOOLS_DIR} -type f -name "loclx" 2>/dev/null | head -n 1`.strip
      
      if !loclx_bin.empty?
        if loclx_bin != TOOLS[:loclx]
          exec_silent("mv #{loclx_bin} #{TOOLS[:loclx]}")
        end
...
class TunnelManager
  ...
  def start_ngrok
    ...
    8.times do  # Increased retries
      if File.exist?(log_file)
        ...
        # Look specifically for ngrok-free.app, ngrok.app, ngrok.dev or ngrok.io URLs
        url = `grep -oE "https://[a-zA-Z0-9-]+\\.ngrok(-free)?\\.(app|dev|io)" #{log_file} 2>/dev/null | head -1`.strip
  ...
  def start_loclx
    ...
    cmd = "#{TOOLS[:loclx]} tunnel http --to 127.0.0.1:#{@port}"
    if @config.has?('loclx_token')
      # Note: The --token flag is removed from recent Loclx CLI versions
      # Users need to authenticate via `loclx account login` interactively.
    end
